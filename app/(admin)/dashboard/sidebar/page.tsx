...
export default function NavCMSPage() {
    ...
    const TypeBadge = ({ item }: { item: NavItem }) => {
        if (item.groupName) return <span className="text-xs font-bold px-2 py-1 rounded-lg bg-purple-400/10 text-purple-400 whitespace-nowrap">Group Header</span>
        if (item.children.length > 0) return <span className="text-xs font-bold px-2 py-1 rounded-lg bg-blue-400/10 text-blue-400 whitespace-nowrap">Parent Menu</span>
        return <span className="text-xs font-bold px-2 py-1 rounded-lg bg-yellow-400/10 text-yellow-400 whitespace-nowrap">Menu Item</span>
    }

    const HrefDisplay = ({ item }: { item: NavItem }) => {
        if (item.groupName) return <span className="text-white/20 text-xs italic whitespace-nowrap">— section label only —</span>
        if (!item.href) return <span className="text-white/20 text-xs italic whitespace-nowrap">— no link —</span>
        return <span className="text-white/40 text-xs font-mono whitespace-nowrap">{item.href}</span>
    }
    ...
    return (
        <div className="p-4 sm:p-8 bg-black min-h-screen">
            ...
            {items.length === 0 ? (
                ...
                // ── FIX: overflow-x-auto supaya bisa scroll horizontal di mobile ──
                <div className="bg-zinc-950 rounded-3xl border border-white/5 overflow-hidden">
                    <div className="overflow-x-auto">
                        <table className="w-full min-w-[640px]">
                            <thead>
                                <tr className="border-b border-white/5">
                                    <th className="text-left px-6 py-4 text-white/30 text-xs font-black tracking-widest whitespace-nowrap">MENU</th>
                                    <th className="text-left px-6 py-4 text-white/30 text-xs font-black tracking-widest whitespace-nowrap">TIPE</th>
                                    <th className="text-left px-6 py-4 text-white/30 text-xs font-black tracking-widest whitespace-nowrap">URL / INFO</th>
                                    <th className="text-left px-6 py-4 text-white/30 text-xs font-black tracking-widest whitespace-nowrap">STATUS</th>
                                    <th className="text-right px-6 py-4 text-white/30 text-xs font-black tracking-widest whitespace-nowrap">AKSI</th>
                                </tr>
                            </thead>
                            <tbody>
                                {items.map((item) => (
                                        ...
                                        <tr className={`border-b border-white/5 hover:bg-white/2 transition-colors ${!item.isActive ? 'opacity-40' : ''}`}>
                                            <td className="px-6 py-4">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div>
                                                        <p className="text-white font-black text-sm whitespace-nowrap">{item.label}</p>
                                                        <p className="text-white/30 text-xs whitespace-nowrap">icon: {item.icon}</p>
                                                    </div>
                                                </div>
                                            </td>
                                            ...
                                            <td className="px-6 py-4">
                                                <button onClick={() => toggleItemActive(item)}
                                                    className={`px-3 py-1 rounded-lg text-xs font-bold transition-all whitespace-nowrap ${item.isActive ? 'bg-green-500/20 text-green-400' : 'bg-white/5 text-white/30'}`}>
                                                    ...
                                                    <tr key={child.id} className="border-b border-white/3 bg-white/1 hover:bg-white/3 transition-colors">
                                                        <td className="px-6 py-3">
                                                            <div className="flex items-center gap-3 pl-8">
                                                                ...
                                                                <div>
                                                                    <p className="text-white/80 font-bold text-sm whitespace-nowrap">{child.label}</p>
                                                                    <p className="text-white/20 text-xs whitespace-nowrap">icon: {child.icon}</p>
                                                                </div>
                                                            </div>
                                                        </td>
                                                        <td className="px-6 py-3">
                                                            <span className="text-xs font-bold px-2 py-1 rounded-lg bg-white/5 text-white/30 whitespace-nowrap">Sub-Menu</span>
                                                        </td>
                                                        <td className="px-6 py-3">
                                                        ...
                                                        <td className="px-6 py-3">
                                                            <span className={`text-xs font-bold px-2 py-1 rounded-lg whitespace-nowrap ${child.isActive ? 'bg-green-500/10 text-green-400' : 'bg-white/5 text-white/20'}`}>
                                                ...
                                                {showAddChild === item.id ? (
                                                    <tr className="border-b border-white/5 bg-yellow-400/3">
                                                        <td colSpan={5} className="px-6 py-3">
                                                            <div className="pl-8 flex items-center gap-3 flex-wrap sm:flex-nowrap">
                                                                <input value={newChild.label} onChange={e => setNewChild(p => ({ ...p, label: e.target.value }))}
                                                                    placeholder="Label sub-menu"
                                                                    className="bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-white text-xs focus:outline-none focus:border-yellow-400/50 w-36" />
                                                                ...
                                                                <input value={newChild.href} onChange={e => setNewChild(p => ({ ...p, href: e.target.value }))}
                                                                    ...
                                                                    className="bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-white text-xs focus:outline-none focus:border-yellow-400/50 flex-1 min-w-[120px]" />
