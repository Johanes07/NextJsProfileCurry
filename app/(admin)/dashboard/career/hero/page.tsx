...
import { Briefcase, Save, Eye, EyeOff, RefreshCw, CheckCircle, AlertCircle, Upload, X, Loader2 } from 'lucide-react'
...
const DEFAULT_DATA: CareerHeroData = {
...
function Field({ label, value, onChange, placeholder, hint, mono = false, textarea = false }: {
    ...
    const baseClass = `w-full bg-zinc-900 border border-zinc-700/60 rounded-xl px-4 py-3 text-white
...
// ── Live Preview ──────────────────────────────────────────────
function LivePreview({ data, isDark }: { data: CareerHeroData; isDark: boolean }) {
    if (isDark) {
        return (
    ...
    return (
        <section className="relative min-h-[280px] overflow-hidden bg-amber-50 flex items-center rounded-2xl">
            ...
            <div className="absolute bottom-0 left-0 w-36 h-36 bg-orange-300/15 rounded-full blur-3xl" />
...
// ── Main CMS ──────────────────────────────────────────────────
export default function CareerHeroCMS() {
    const [data, setData] = useState<CareerHeroData>(DEFAULT_DATA)
    ...
    const originalRef = useRef<CareerHeroData>(DEFAULT_DATA)
    const fileInputRef = useRef<HTMLInputElement>(null)

    useEffect(() => {
        fetch('/api/admin/career/hero')
            .then(r => r.json())
    ...
    const set = (key: keyof CareerHeroData) => (val: string) =>
    ...
    async function handleSave() {
        setSaving(true)
        try {
            const res = await fetch('/api/admin/career/hero', {
                method: 'PUT',
            ...
            <div className="sticky top-0 z-40 bg-zinc-950/95 backdrop-blur-sm border-b border-zinc-800/60">
                <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h1 className="text-base font-bold text-white">Career Hero</h1>
                            <p className="text-xs text-zinc-500">Edit konten section hero halaman career</p>
                        </div>
                    </div>
                    <div className="flex items-center gap-3">
                        {isDirty && (
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="bg-zinc-900 border border-zinc-800/60 rounded-2xl p-6">
                            ...
                            <Field label="Teks Badge" value={data.badgeText} onChange={set('badgeText')}
                                placeholder="JOIN THE TEAM" hint="Teks yang muncul di badge atas heading. Gunakan HURUF KAPITAL." mono />
                        </div>
                        ...
                        <div className="bg-zinc-900 border border-zinc-800/60 rounded-2xl p-6">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <Field label="Baris Pertama" value={data.headingLine1} onChange={set('headingLine1')}
                                    placeholder="WORK WITH" hint="Warna putih (dark) / abu gelap (light)" mono />
                                <Field label="Baris Kedua" value={data.headingLine2} onChange={set('headingLine2')}
                                    placeholder="US" hint="Warna kuning / aksen" mono />
                            </div>
                        ...
                        <div className="bg-zinc-900 border border-zinc-800/60 rounded-2xl p-6">
                            ...
                            <Field label="Teks Subtitle" value={data.subtitle} onChange={set('subtitle')}
                                placeholder="Be part of something obsessive..."
                                hint="Deskripsi singkat di bawah heading. Maks ~150 karakter direkomendasikan." textarea />
                            ...
                            {data.imageUrl ? (
                                ...
                                <div onClick={() => fileInputRef.current?.click()}
                                    ...
                                    className={`relative w-full h-48 rounded-2xl border-2 border-dashed cursor-pointer
                                        ...
                                        ${dragOver ? 'border-yellow-400 bg-yellow-400/5 scale-[1.01]' : 'border-zinc-700 hover:border-zinc-500 hover:bg-zinc-800/50'}
                        ...
                        <div className="sticky top-24">
                            <div className="flex items-center justify-between mb-4">
                                ...
                                <button onClick={() => setPreviewDark(p => !p)}
                                    className="flex items-center gap-2 text-xs text-zinc-500 hover:text-zinc-300 transition-colors bg-zinc-800/60 border border-zinc-700/50 rounded-xl px-3 py-2">
                            ...
                            <div className="rounded-2xl overflow-hidden border border-zinc-800/60 shadow-2xl shadow-black/50">
                                <div className="bg-zinc-900/50 border-b border-zinc-800/60 px-4 py-2.5 flex items-center gap-2">
                                    ...
                                    <span className="text-xs text-zinc-600 ml-2 font-mono">
                                        yoursite.com/career — {previewDark ? '🌙 dark' : '☀️ light'}
                                    </span>
                                </div>
